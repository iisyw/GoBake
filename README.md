...
- **自定义 Go 命令**：支持选择不同版本的 Go 编译器（如 go120、go123 等）进行构建
...
3. 按照交互式提示进行选择：
   ...
   - 选择 Go 命令：可以选择不同版本的 Go 编译器，如 go120、go123 等
...
# 使用命令行参数进行构建（待实现）
gobake --output=./dist --cgo=0 --go-command=go120 --platforms=windows/amd64,linux/amd64
```
...
────────────────────────────────────────────────────────────────────────────────
是否使用默认Go命令? (当前: go, Y/n): n
您可以指定使用特定的Go版本命令，例如go120、go123等
请输入Go命令: go120
Go命令设置为: go120
...
GO命令      = go120
────────────────────────────────────────────────────────────────────────────────
执行构建命令: go120 build -ldflags "-w -s" -o "./build/myproject_windows_amd64.exe"
...
A: GoBake 支持选择自定义的 Go 命令。如果您在系统中安装了多个 Go 版本，可以在运行时指定要使用的具体命令（如 go120、go123）。确保您指定的命令在系统 PATH 中可用。
